...
window.addEventListener(
  ...
  function stopAllAudio({ hide = true, reset = true } = {}) {
    if (window.speechSynthesis) window.speechSynthesis.cancel();
    if (typeof activeAudioCleanup === "function") activeAudioCleanup({ hide, reset });
    activeAudioCleanup = null;
  }
  window.stopAllNovoAyitiAudio = stopAllAudio;
  window.addEventListener("pagehide", () => stopAllAudio({ hide: true, reset: true }));
  window.addEventListener("beforeunload", () => stopAllAudio({ hide: true, reset: true }));
...
function renderChapter(id) {
    ...
    <div class="container book-container">
      ...
      <article class="chapter-page">
        ...
        <section class="chapter-actions" aria-label="Aksyon pou lektè yo">
          <div class="download-panel"><strong>Telechaje pati liv la</strong><p>Li PDF ki koresponn ak chapit sa a.</p><div class="download-links"><a class="text-link" href="/downloads/nouvo-ayiti-pati-1.pdf" download>Pati I</a><a class="text-link" href="/downloads/nouvo-ayiti-pati-2.pdf" download>Pati II</a><a class="text-link" href="/downloads/nouvo-ayiti-pati-3.pdf" download>Pati III</a><a class="text-link" href="/downloads/nouvo-ayiti-pati-4.pdf" download>Pati IV</a></div></div>
  ...
  const loadVoices = () => {
    const allowedVoices = window.speechSynthesis.getVoices().filter((voice) => /^(ht|fr|en)([-_]|$)/i.test(voice.lang));
    audioState.voices = allowedVoices.sort((a, b) => { const rank = (voice) => /^ht([-_]|$)/i.test(voice.lang) ? 0 : /^fr([-_]|$)/i.test(voice.lang) ? 1 : 2; return rank(a) - rank(b) || a.name.localeCompare(b.name); });
    voiceSelect.innerHTML = audioState.voices.map((voice, index) => `<option value="${index}">${esc(cleanVoiceName(voice))}</option>`).join("");
    if (audioState.voices.length) voiceSelect.value = "0";
  ...
  const resetAudio = () => { audioState.index = 0; audioState.savedResumeIndex = 0; audioState.utterance = null; sessionStorage.removeItem(resumeKey); updateProgress(); audioTrigger.textContent = "Koute chapit la"; audioTrigger.setAttribute("aria-expanded", "false"); };
  const stopAudio = ({ hide = false, reset = true } = {}) => { if (supported) window.speechSynthesis.cancel(); if (reset) resetAudio(); if (hide) player.classList.remove("is-visible"); };
  const speakCurrent = () => { if (!supported || window.speechSynthesis.speaking || audioState.index >= chapterBlocks.length) return; const utterance = new SpeechSynthesisUtterance(chapterBlocks[audioState.index]); const voice = audioState.voices[Number(voiceSelect.value)]; if (voice) { utterance.voice = voice; utterance.lang = voice.lang; } else utterance.lang = "fr-FR"; utterance.rate = Number(player.querySelector("#audioRate").value); utterance.onstart = () => { sectionLabel.textContent = `Seksyon ${audioState.index + 1} / ${chapterBlocks.length}`; }; utterance.onend = () => { audioState.index += 1; audioState.utterance = null; updateProgress(); if (audioState.index < chapterBlocks.length) speakCurrent(); else audioTrigger.textContent = "Koute chapit la"; }; utterance.onerror = () => { audioState.utterance = null; sectionLabel.textContent = "Odyo pa disponib sou aparèy sa a"; audioTrigger.textContent = "Koute chapit la"; }; audioState.utterance = utterance; updateProgress(); window.speechSynthesis.speak(utterance); };
  if (!supported) { player.innerHTML = `<p class="audio-unavailable">Navigatè sa a pa sipòte lekti vwa. Tanpri eseye yon navigatè ki sipòte Web Speech API.</p>`; audioTrigger.disabled = true; } else { loadVoices(); window.speechSynthesis.addEventListener("voiceschanged", loadVoices); audioTrigger.addEventListener("click", () => { player.classList.add("is-visible"); audioTrigger.setAttribute("aria-expanded", "true"); if (!window.speechSynthesis.speaking) { audioState.index = 0; speakCurrent(); } }); player.querySelector("#audioPlay").addEventListener("click", () => { if (!window.speechSynthesis.speaking) { audioState.index = 0; speakCurrent(); } }); player.querySelector("#audioPause").addEventListener("click", () => window.speechSynthesis.pause()); player.querySelector("#audioResume").addEventListener("click", () => { const stored = Number(sessionStorage.getItem(resumeKey)); if (Number.isInteger(stored) && stored >= 0 && stored < chapterBlocks.length) audioState.index = stored; audioState.savedResumeIndex = audioState.index; sessionStorage.setItem(resumeKey, String(audioState.index)); if (!window.speechSynthesis.speaking) speakCurrent(); else window.speechSynthesis.resume(); }); player.querySelector("#audioStop").addEventListener("click", () => stopAudio({ reset: true })); player.querySelector("#audioClose").addEventListener("click", () => stopAudio({ hide: true, reset: true })); player.querySelector("#audioRate").addEventListener("change", () => { if (audioState.utterance) { window.speechSynthesis.cancel(); speakCurrent(); } updateProgress(); }); voiceSelect.addEventListener("change", () => { voiceStatus.textContent = audioState.voices[Number(voiceSelect.value)] ? cleanVoiceName(audioState.voices[Number(voiceSelect.value)]) : "Vwa aparèy la pa disponib"; if (window.speechSynthesis.speaking) { window.speechSynthesis.cancel(); speakCurrent(); } }); activeAudioCleanup = ({ hide = true, reset = true } = {}) => { stopAudio({ hide, reset }); window.speechSynthesis.removeEventListener("voiceschanged", loadVoices); }; updateProgress(); }
...
const PILLARS = [
  ...
  ["enfrastrikti", "Enfrastrikti", "Wout, dlo, kouran ak transpò.", [25, 26, 27, 28], ["enfrastrikti", "transpò", "wout", "dlo", "elektrisite", "enerji"]],
  ["agrikilti", "Agrikilti", "Irigasyon, pwodiksyon lokal ak manje.", [13, 14], ["agrikilti", "irigasyon", "alimantè", "riral", "pwodiksyon"]],
  ["anviwonman", "Anviwònman", "Rebwazman, dlo ak rezilyans klimatik.", [25, 27, 28, 29], ["anviwònman", "klima", "dechè", "rebo", "rezilyans", "renouvlab"]],
  ["dijital", "Dijitalizasyon ak inovasyon", "Pòtal Leta ak sèvis piblik sou entènèt.", [1, 10, 26], ["dijital", "teknoloji", "syantifik", "inovasyon", "done", "peman elektwonik"]]
];
function related(p) {
  const ids = new Set(p[3]);
  const patterns = p[4].map((term) => new RegExp(term, "i"));
  return CHAPTERS.filter((chapter) => ids.has(Number(chapter.id)) || patterns.some((pattern) => pattern.test(chapter.title)));
}
...
function renderPillar(id){const p=PILLARS.find(x=>x[0]===id);if(!p)return renderNotFound();const rows=related(p);mount.innerHTML=`<div class="container book-container">${breadcrumb([HOME_CRUMB,{label:"10 Pilye",href:"#route-pilye"},{label:p[1]}])}${pageHead("Pilye estratejik",p[1],`${p[2]} ${rows.length} chapit ki gen rapò.`)}<h2 class="page-subtitle">Chapit ki gen rapò</h2><ul class="toc-list">${rows.map(c=>`<li class="toc-item"><a href="#route-chapit-${c.id}"><span class="toc-num">${esc(c.num)}</span><span class="toc-text"><strong>${esc(c.title)}</strong><small>${esc(c.summary)}</small></span><span class="toc-go">Li chapit la</span></a></li>`).join("")}</ul></div>`}
...
function renderDownloads(){const parts=[1,2,3,4];mount.innerHTML=`<div class="container book-container">${breadcrumb([HOME_CRUMB,{label:"Telechaje"}])}${pageHead("Sant telechajman","Telechaje Liv la","Kat pati PDF yo disponib pou lekti, pataj ak achiv.")}<div class="cards-grid">${parts.map((part)=>`<article class="pillar-card"><h2>Pati ${["I","II","III","IV"][part-1]}</h2><p>PDF ofisyèl Nouvo Ayiti pou pati sa a.</p><a class="btn btn-primary" href="/downloads/nouvo-ayiti-pati-${part}.pdf" download>Telechaje PDF</a></article>`).join("")}<article class="pillar-card"><h2>Audiobook</h2><p>Vèsyon odyo liv la ap disponib nan yon pwochen etap.</p><span>Ap vini</span></article><article class="pillar-card"><h2>EPUB</h2><p>Fòma EPUB la ap disponib apre koreksyon final yo.</p><span>Ap vini</span></article></div></div>`}
...
function exitPage() {
  stopAllAudio({ hide: true, reset: true });
